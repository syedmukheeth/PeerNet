...
const bootstrap = async () => {
    ...
    // We start these in parallel WITHOUT awaiting them before listen()
    // This ensures Render detects the port is open ASAP.
    ...
    // Binding the port is the priority for cloud deployment
    httpServer.listen(PORT, () => {
        logger.info(`✨ PeerNet server live on port ${PORT} [${process.env.NODE_ENV}]`);
        // Start background tasks AFTER port is bound
        initServices();
    });
